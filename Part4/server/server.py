...
# Setting the maximum bandwidth limit to 3 Gbps
MAX_BW_LIMIT = 3
...
def getdata(FileName, dataconn):
    ...
    with open(FileName, 'rb') as file:
        ...
        # Sending the file in small packets 
        packet_size = 134217728
        ...
        while data:
            ...
            # Measuring the transmission rate and transmission progress
            start = time.process_time_ns()
            dataconn.send(data)
            end = time.process_time_ns()
            
            # Avoiding the bandwidth to exceed the maximum bandwidth limit
            if ((packet_size / (end - start)) > MAX_BW_LIMIT):
                timeout = ((packet_size / MAX_BW_LIMIT) + start - end) / 10**9
                time.sleep(timeout)
                end += timeout * 10**9
                
            data = file.read(packet_size)
                
            rate = packet_size / (end - start)
            print("Transmission Rate = " + str(rate) + " " + "Gbps")
            progress = packet_size * n * 100 / total_size
            print("Transmission Progress = " + str(progress) + " " + "%")
